...
export async function generateSceneOutlinesFromRequirements(
  ...
  let availableImagesText =
    requirements.language === 'es' ? 'No hay imágenes disponibles' : 'No images available';
  ...
  const prompts = buildPrompt(PROMPT_IDS.REQUIREMENTS_TO_OUTLINES, {
    ...
    pdfContent: pdfText
      ...
      : requirements.language === 'es'
        ? 'Ninguno'
        : 'None',    availableImages: availableImagesText,
    userProfile: userProfileText,
    mediaGenerationPolicy,
    researchContext:
      options?.researchContext || (requirements.language === 'es' ? 'Ninguno' : 'None'),
